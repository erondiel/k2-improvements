...
| v1.1.13 | (this release) | **Local-mode detection.** When bootstrap is invoked on the printer itself (PRINTER_IP matches localhost / 127.0.0.1 / a local interface IP / hostname), `remote()` runs commands locally via `sh -c` instead of via SSH. Skips the entire dropbear / sshpass / expect-wrapper / password-prompt chain — no auth needed when source and target are the same machine. Also faster (no network roundtrip per command). Users running bootstrap from their PC see zero behavior change. Reported by erondiel as the cleanest way to handle "running bootstrap from inside the printer" use case (DiodeKing's testing flow). |
