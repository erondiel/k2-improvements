...
| v1.1.8 | (this release) | Two more sshpass-expect wrapper fixes found via real bootstrap-style integration testing (DiodeKing's run on a K2 Plus targeting another K2 Plus): (1) `log_user 1` was forwarding pre-auth output (ssh option-ignoring warnings, fingerprint prompts, password prompt) to expect's stdout, which `$(remote "...")` then captured — `PRINTER_FW` ended up as "ssh: Ignoring unknown configuration option..." instead of empty/version-string. Fix: `log_user 0` initially, flip to `1` only after sending password. (2) Pty's CRLF translation left a trailing `\r` on captured output that `$()` doesn't strip; bootstrap's `case "$PRINTER_FW" in 1.1.5.2)` then failed to match. Fix: pipe wrapper output through `tr -d '\r'`, with exit code preserved via a tmp file (busybox ash has no PIPESTATUS). End-to-end verified: bootstrap-style `$()` capture now produces clean values that match case statements. |
