...
| v1.1.9 | (this release) | Bootstrap now refreshes the expect-based sshpass wrapper from GitHub on every run when it detects our wrapper is already installed. Without this, users who installed v1.1.6's broken wrapper continue using the broken version on subsequent runs because `command -v sshpass` finds `/opt/bin/sshpass` and skips the install path entirely. Detection uses a marker comment in the wrapper's header. Real sshpass binaries don't match the marker so the refresh logic doesn't touch them. |
