...
is_better_root()   { getent passwd root 2>/dev/null | grep -q ':/mnt/UDISK/root:' ; }
is_cartographer()  { grep -q '^\[cartographer\]' "$PRINTER_CFG_DIR/printer.cfg" 2>/dev/null; }
is_moonraker()     { [ -d /mnt/UDISK/printer_data/moonraker ] || [ -f /mnt/UDISK/printer_data/config/moonraker.conf ]; }
is_fluidd()        { [ -d /mnt/UDISK/fluidd ] || [ -f /mnt/UDISK/printer_data/config/fluidd.cfg ]; }
...
is_jimmyv()        { grep -qE 'y_offset:\s*36' "$PRINTER_CFG_DIR/printer.cfg" 2>/dev/null; }
